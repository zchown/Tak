...
void test_makeMove() {
    ...
    CU_ASSERT_EQUAL(result, SUCCESS);
    CU_ASSERT_EQUAL(SQ_HEAD(readSquare(state->board, SET_POS(0, 0))).stone, FLAT);
    freeGameState(state);
    ...
    CU_ASSERT_EQUAL(result, SUCCESS);
    CU_ASSERT_EQUAL(SQ_HEAD(readSquare(state->board, SET_POS(3, 2))).stone, FLAT);
    freeGameState(state);
}
...
void test_undoMove() {
    ...
    CU_ASSERT_EQUAL(readSquare(state->board, SET_POS(0, 0))->numPieces, 0);
    freeGameState(state);
    ...
    undoMoveNoChecks(state, &move, true);
    CU_ASSERT_EQUAL(SQ_HEAD(readSquare(state->board, SET_POS(3, 2))).stone, FLAT);
    CU_ASSERT_EQUAL(readSquare(state->board, SET_POS(2, 3))->numPieces, 0);
    freeGameState(state);
}

// test not updated for MoveList
